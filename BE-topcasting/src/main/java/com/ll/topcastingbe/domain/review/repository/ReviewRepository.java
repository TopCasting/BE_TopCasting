...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("select r from Review r where r.orderProduct.option.product.id = :productId and r.rating = :rating")
    ...
    @Query("select r from Review r where r.orderProduct.option.product.id =:productId order by r.rating desc ")
    List<Review> findByProductIdOrderByRatingDesc(@Param("productId") Long productId);

    @Query("select r from Review r where r.orderProduct.option.product.id =:productId order by r.rating ASC ")
    List<Review> findByProductIdOrderByRatingAsc(@Param("productId") Long productId);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.orderProduct = :orderProduct")
    Long countReviewsByOrderProduct(@Param("orderProduct") OrderProduct orderProduct);
