...
public interface CartOptionRepository extends JpaRepository<CartOption, Long> {
    ...
    @Query("select co from CartOption co join fetch co.option op join fetch op.product it where co.cart.id = :cartId")
    ...
    @Query("select co from CartOption co join fetch co.cart c where co.id = :cartProductId")
    Optional<CartOption> findByIdWithMember(@Param("cartProductId") Long cartProductId);
